...
```{r}
A1_nts<-readland.nts("A1.nts")
```
```{r}
A2_nts<-readland.nts("A2.nts")
```
Read the nts files in to create a 3D array. 
```{r}
filelist<-(c("A1.nts", "A2.nts"))
filelist
A1_A2_nts<-readmulti.nts(filelist)
```
...
- creating a surface from the digitfix
```{r}
d_surf<-digitsurface(A1, A1_nts,1, center = TRUE) #not sure why this doesnt work
```

...
digit.fixed is used for selecting fixed landmarks that do not change from specimen to specimen. digitsurface can then be used to place equal spaced landmarks between those points to represent a surface, while digit.curve and define.sliders can be used to fit curves on the specimen (I haven't really figured out surface and curve yet)
...
```{r}
getwd()
A1_nts<-readland.nts("A1.nts")#I changed the name so they would not be overwritten
```
```{r}
...
tar.link.lwd=2), method="vector", links = scallinks)
```
```{r}
...
```{r}
define.sliders(A1_nts, 2, surfsliders=T, write.file=T)
```
```{r}
plotspec(A1_nts, slide)
```

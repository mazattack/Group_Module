...
```{r setup, echo =FALSE}
knitr::opts_chunk$set(cache = TRUE, autodep = FALSE)
```
# Welcome, friends! 
...
In the base R graphics system, the function persp() draws perspective plots of a surface over the x–y plane. Set eval=TRUE to run demo(persp). The demo will give you an idea of what this function can do. 
...
The plot3D package builds on on persp() to provide functions for both 2D and 3D plotting. Load the plot3D package and set eval=TRUE go through some pretty impressive plot examples:
...
The scatterplot3d package is the "go-to" package for simple, non-interactive 3D scatter plots. Load this package and set eval=TRUE to go through some examples of spirals, surfaces and 3D scatterplots: 
...
>>>>>>> 26c1a90b1f2c735fd9349462e2917b38ce980ed8
```{r}
...
The lattice package has its own distinctive look. Once you see one lattice plot it should be pretty easy to distinguish plots made with this package from base graphics plots. Load the package and set eval=TRUE to see a 3D graph of a volcano, and 3D surface and scatter plots of iris data. 
...
### Install packages
Let's first set eval=TRUE to install the packages that you'll need for this section: 
...
You can set eval=TRUE to install the package from CRAN.
```{r, eval=FALSE}
...
Set eval=TRUE if you need to install these packages. 
...
##Install geomorph
To install "geomorph", making sure dependencies = TRUE so that it installs packages that geomorph depends on also. 
...
The Geomorph package allows you to input 3D data in ASCII ".ply" format which is a common output from 3D scanners. Here you will load three 3D scans of archaeological arrow heads from an online repository of 3D models [âsketchfabâ](https://sketchfab.com/demoon/collections/ref_weapon_neolit)
...
We can not only calculare ER differences between defined groups of species, but estimate whether divergence of sister species is equivalent to divergence between non-sister species using analysis of phylogenetic signal. This is an estimation of the degree of covariation in phenotype due to shared ancestry. The mean head shape data points among the Plethodon species shouldn't be considered independent data because they all have shared evolutionary history and some species groups are more closely related than others. Therefore, geomorph provides a simple function for estimating the strength of phylogenetic signal in high dimensional data.
...
For an overview of all of Geomorph's functions and datasets, see the Package Documentation by Adams et al (2017)

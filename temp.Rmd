...
```{r}
plotspec(A1, A1_temp, ptsize=5, centered=T)
```
DEFINE CURVE SLIDERS HERE IF TIME ALLOWS
...
We can also run the digitsurface using exisiting landmark data. Let's use an nts file I created for A3. First read in the data. Since we are working with ".nts" files, we can use readland.nts to load the data. *NEED TO PUT DATA HERE*

```{r}
A3_nts<- readland.nts(curl("https://raw.githubusercontent.com/mazattack/Group_Module/master/Data/A3.nts"))
digitsurface(A3, A3_nts, 1, TRUE)
```
 
Now that we have imported all our 3D models, loaded and created the fixed and semi-landmarks, we need to transform them into a format for geomorphometric analysis. When loading data from multiple landmark files of individual specimens, we can use "readmulti.nts" to combine filed into a 3D array for analysis. This is easiest if the file is in the working directory

If you have a single .nts file with the landmarks of multiple speciments use readlands.nts to import.
If you have mulitple .nts files with the landmarks of a single specimen use readmulti.nts
To import multiple .nts files with multiple specimens or multiple .tps files, you can use the funtion below, specifying 'patterns = ".nts" or ".tps"' and 'tmp <- "readland.tps" or "readland.nts"' as appropriate.
...
To import multiple tps files, there is no option to import them as a group, so we need to use a loop function. The following function uses .nts files so you can see how the function works. Simply replace the two ".nts" sections with ".tps". 
...
Here we will use readmulti.nts to load the three .nts files we created containing the fixed and semi-fixed landmarks. 
```{r}
setwd("C:/Users/Maria/Documents/GitHub/Group_Module")
